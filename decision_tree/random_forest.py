...
def create_shadow_features(features):
    ...
    for i in range(num_of_features):
        shadow_x = np.concatenate([shadow_x, rng.permutation(features[:, i]).reshape((len_of_features, 1))], axis=1)
    return shadow_x
...
class RandomForest:
    ...
    def fit(self, x, y):
        ...
        # create trees
        self.trees = [Tree(self.num_of_class, max_depth=self.max_depth) for _ in range(self.n_estimators)]
    ...
    def fit_with_boruta(self, x, y):
        ...
        for _ in range(100):
            ...
            feature_importance = np.array([0.] * num_of_features)
            for tree in self.trees:
                feature_importance += tree.feature_importance
            feature_importance /= len(self.trees)
            feature_importances = np.concatenate([feature_importances, 
                                                  feature_importance.reshape(num_of_features, 1)], axis=1)
